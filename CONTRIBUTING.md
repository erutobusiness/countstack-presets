...
Each preset lives in its own directory under `presets/` (e.g., `presets/my-game/preset.json`). The JSON file must follow the schema defined in `schema/preset.schema.json`.
...
| `labels` | object | Optional custom labels for database tabs |
...
| `notes` | array | Structured notes with localized text and optional URLs |
...
| Field | Type | Description |
|-------|------|-------------|
| `entries` | LocalizedString | Custom label for database entries tab |
| `groups` | LocalizedString | Custom label for database groups tab |
| `notes` | LocalizedString | Custom label for notes tab |
...
| `unit` | LocalizedString | Optional display unit (e.g., `{ "en": "sets" }`) |
...
Structured notes with localized text and optional external reference URLs.

| Field | Type | Description |
|-------|------|-------------|
| `text` | LocalizedString | Note content |
| `url` | string | Optional URL for external reference (opens in new tab) |

Example:

```json
{
  "text": { "ja": "重要なヒント", "en": "An important tip" },
...
```json
{
  ...
  "notes": [
    {
      "text": { "en": "A helpful tip for this preset." }
    }
  ]
}
```
...
{
  "id": "my-game",
  "name": { "ja": "マイゲーム", "en": "My Game Preset" },
  "description": { "ja": "説明文", "en": "Description of the preset" },
  "version": "1.0.0",
  "size": 0,
  "counters": 1,
  "entryCount": 1,
  "groupCount": 0,
  "dataUrl": "https://erutobusiness.github.io/countstack-presets/presets/my-game/preset.json",
  "tags": ["tag1", "tag2"]
}
```
...
- `dataUrl` must follow the pattern: `https://erutobusiness.github.io/countstack-presets/presets/<id>/preset.json`
...
3. **Add your files**:
   - `presets/<preset-id>/preset.json` — the preset data
