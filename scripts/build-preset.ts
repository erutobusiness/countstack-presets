...
function main() {
  ...
  if (frlgEntry) {
    ...
    frlgEntry.pokemonCount = pokemon.length;
    frlgEntry.trainerCount = preset.database.trainers.length;
    registry.updatedAt = new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
    writeFileSync(REGISTRY_PATH, JSON.stringify(registry, null, 2));
  }

  console.log(`Built preset: ${pokemon.length} Pokémon, ${preset.database.trainers.length} trainers`);
